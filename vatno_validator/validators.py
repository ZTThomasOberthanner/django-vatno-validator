...
class VATNoValidator(object):
    ...
    regexes = {
        ...
        "CH": _lazy_re_compile(r"^CHE\d{9}$"),  # Switzerland
    ...
    def __call__(self, value):
        ...
        if (value[:2] not in self.regexes) or (value[:2] not in self.allowed_countries):
            raise ValidationError(self.message, code=self.code)
    ...
    def __eq__(self, other):
        return isinstance(other, VATNoValidator) and (self.message == other.message) and (self.code == other.code)
